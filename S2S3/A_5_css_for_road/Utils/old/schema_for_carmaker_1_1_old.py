...
class Makejson():
    ...
    def __init__(self,_xodr_dir, simulation_name): 
        self.xodr_file_path, self.xodr_file = self.get_xodr_file(_xodr_dir, simulation_name)
        self.parsed_config = self.parse_config(self.xodr_file)
        ### 여기서부터는 수정 필요
        ...
        parsed_config = self.parse_config(self.xodr_file)
        print()
    # 파싱 함수
    ...
    def get_xodr_file(self, _xodr_dir, simulation_name):
      ...
      # 도로 정보 추출
    def extract_lane_info(self, xodr_content):
        root = ET.ElementTree(ET.fromstring(xodr_content)).getroot()
        lanes = []
        ...
        for lane_section in root.findall(".//laneSection"):
            for lane in lane_section.findall(".//lane"):
                ...
                curvature_element = lane.find(".//curvature")

                # width와 curvature 값을 안전하게 추출
                width = float(width_element.get("value")) if width_element is not None else None
                curvature = float(curvature_element.get("value")) if curvature_element is not None else None
